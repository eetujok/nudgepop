...
export async function onSuccess({ params, record, logger, api, connections }) {
  switch(record.topic) {
    ...
    const filter = {
      ...
      await api.internal.popupConditions.deleteMany({ filter });
      await api.internal.popupContent.deleteMany({ filter });
      await api.internal.popupStyling.deleteMany({ filter });
      break;
  }
};
